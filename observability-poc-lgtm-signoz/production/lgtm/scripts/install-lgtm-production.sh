...
if grep -R "REPLACE_ME" "${PROD_DIR}/values" >/dev/null 2>&1; then
  echo "ERROR: production/lgtm/values still contains REPLACE_ME placeholders."
...
if ! kubectl -n "${NAMESPACE}" get secret lgtm-object-storage >/dev/null 2>&1; then
  ...
  echo "Create it from your secret manager. See production/lgtm/secrets/object-storage.example.yaml."
  exit 1
fi

if ! kubectl -n "${NAMESPACE}" get secret grafana-admin >/dev/null 2>&1; then
  echo "ERROR: secret '${NAMESPACE}/grafana-admin' not found."
  echo "Create it from your secret manager. See production/lgtm/secrets/grafana-admin.example.yaml."
  exit 1
fi
